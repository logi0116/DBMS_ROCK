...
SELECT * FROM EMP WHERE MONTHS_BETWEEN(SYSDATE, HIREDATE) <500;

--퀴즈 1-1. 입사일로부터 32년이 지난 사원만 출력해보기
SELECT * FROM EMP WHERE MONTHS_BETWEEN(SYSDATE, HIREDATE) > 384;
SELECT * FROM EMP WHERE MONTHS_BETWEEN(SYSDATE, HIREDATE) > 530;
...
SELECT ENAME, HIREDATE, NEXT_DAY(HIREDATE, '월요일') AS "고용일 다음 월요일" FROM EMP;


--퀴즈 3. 사원의 입사일을 기준으로 월 단위로 반올림해서 출력해보기
SELECT ENAME, HIREDATE AS "입사일", ROUND(HIREDATE, 'MONTH') AS "월단위반올림" FROM EMP;
